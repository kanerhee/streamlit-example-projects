...
def make_alt_barchart(df, offordef):
    ...
    df2 = df.copy()
    # df2 = df2[df2['Offense'] == teamsChosen]
...
def offenseYards(df, PLAYTYPE):
    ...
    df2 = df2.groupby(['Offense', 'Week']).agg({'week':'sum'}).unstack().fillna(0.0)
...
def OffensePlaysOf(df, threshold):

    df2 = df.copy()
    df2 = df2[df2['Yards'] >= threshold]
    df2.rename(columns={'Yards':'week'}, inplace=True)
    df2 = df2.groupby(['Offense', 'Week']).agg({'week':'count'}).unstack().fillna(0)
...
def app():
    ...
    st.write('Offensive Yards per Week - Bar')
    st.write(make_alt_barchart(df.copy(), 'Offense'))

    st.write('Offense Yards per Week - Crosstab')
    st.write(offenseYards(df.copy(), False))

    # st.write('Passing Yards per Week')
    # st.dataframe(offenseYards(df.copy(), 'PASS'))
    #
    # st.write('Rushing Yards per Week')
    # st.write(offenseYards(df.copy(), 'RUSH'))
